...
async function handleHitStone(interaction) {
    ...
    let totalHits = 0;
    let remainingHP = STONE_HP - playerData.stoneProgress;

    while (playerData.stoneProgress < STONE_HP) {
        playerData.stoneProgress += playerData.damageLevel;
        totalHits++;
    }

    playerData.stoneProgress = 0;
    
    const cards = dataManager.getCards();
    const availableCards = stones.default.cards.filter(cardName => cards[cardName]);
    
    if (availableCards.length === 0) {
        return await interaction.reply({ content: 'No valid cards available!', ephemeral: true });
    }
    ...
    const embed = new EmbedBuilder()
        ...
        .setDescription(`You hit the stone **${totalHits} times** and received: **${newCardName}**`)
